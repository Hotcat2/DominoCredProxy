...
{
	public class HttpSocketHeadersReader
	{
		...
		public HttpSocketHeadersReader(ILoggerFactory loggerFactory, string remoteAddress, Socket client)
		{
			_logger = loggerFactory.CreateLogger(typeof(HttpSocketHeadersReader));
		...
		public async IAsyncEnumerable<HttpHeader> HeadersAsync([EnumeratorCancellation] CancellationToken ctx)
		{
			...
			var mem = new Memory<byte>(new byte[256]); // ^^^ 1024
			var sw = Stopwatch.StartNew();
			
			do
			{
				if (headersCompleteChars >= 1) // ^^^ Воркэрунд, хорошо бы это пофиксать.
					break;

				sw.Restart();
				while (_client.Available <= 0) // Waiting until data will be available, wait 3ms if not.
				{
				...
				for (var n = 0; n < bytesRead; n++)
				{
					...
					if (symbolsNewline.Contains(ch) && headersCompleteChars <= 0) // Is line ended.
					{
